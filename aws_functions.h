    ...
    const ngx_str_t *s3_endpoint)
{
	...
	const struct AwsCanonicalRequestDetails canon_request =
		ngx_http_aws_auth__make_canonical_request(pool, r, s3_bucket_name, date, s3_endpoint);
...
static inline const ngx_array_t* ngx_http_aws_auth__sign(ngx_pool_t *pool, ngx_http_request_t *r,
    ...
    const ngx_str_t *s3_endpoint) 
{
	const struct AwsSignedRequestDetails signature_details = ngx_http_aws_auth__compute_signature(pool, r, signing_key, key_scope, s3_bucket_name, s3_endpoint);
