...
static void host_header_ctor(void **state)
{
	...
	host = ngx_http_aws_auth__host_from_bucket(pool, &bucket);
...
static void x_amz_date(void **state)
{
	...
	t = 1;
	date = ngx_http_aws_auth__compute_request_time(pool, &t);
...
static void hmac_sha256(void **state)
{
    ...
    hash = ngx_http_aws_auth__sign_sha256_hex(pool, &text, &key);
...
static void sha256(void **state)
{
    ...
    text.data = "asdf"; text.len=4;
    hash = ngx_http_aws_auth__hash_sha256(pool, &text);
	assert_int_equal(64, hash->len);
...
static void canon_header_string(void **state)
{
    ...
    retval = ngx_http_aws_auth__canonize_headers(pool, NULL, &bucket, &date, &hash, &endpoint);
...
static void canonical_qs_empty(void **state)
{
  ...
  request.connection = NULL;
	...
	const ngx_str_t *canon_qs = ngx_http_aws_auth__canonize_query_string(pool, &request);
...
static void canonical_url_sans_qs(void **state)
{
  ...
  request.connection = NULL;
	...
	const ngx_str_t *canon_url = ngx_http_aws_auth__canon_url(pool, &request);
...
static void canonical_request_sans_qs(void **state)
{
  ...
  request.connection = NULL;
	...
	result = ngx_http_aws_auth__make_canonical_request(pool, &request, &bucket, &aws_date, &endpoint);
...
static void basic_get_signature(void **state)
{
  ...
  request.connection = NULL;
	...
	struct AwsSignedRequestDetails result = ngx_http_aws_auth__compute_signature(pool, &request,
